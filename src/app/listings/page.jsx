...
const Page = () => {
  ...
  useEffect(() => {
    ...
    if (email) {
      axios.get(`http://localhost:4000/api/user/houses/${email}`)
        .then(response => {
  ...
  const handleCardClick = (id) => {
    ...
    // Example: window.location.href = `/house/${id}`;
        ...
        houses.map((house) => (
          <div key={house._id} className="card bg-white rounded-lg shadow-md xl:w-10/12 m-4 hover:shadow-2xl transition-transform transform hover:scale-105 fade-in-up" onClick={() => handleCardClick(house._id)}>
            <div className="image-container">
              <img
                src={house.image[0] || 'default-image-url'} // Use a default image URL if house.image is empty
                alt={house.houseType}
